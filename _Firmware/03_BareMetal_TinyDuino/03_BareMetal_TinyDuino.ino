...
// For error lights
#define LEDPIN 7 // I can't use the built-in LED because it is involved in SPI comms with the SD shield.
...
void ledBlink(int ledPinLocation, int flashes, int evenOn, int evenOff, int oddOn, int oddOff)
{
    ...
    for(int i=0; i<flashes; i++)
    {
        ...
        if (even == false)
            Sleepy::loseSomeTime(evenOn);
        else
            Sleepy::loseSomeTime(oddOn);
            
        digitalWrite(ledPinLocation, LOW);
        
        if (even == false)
            Sleepy::loseSomeTime(evenOff);
        else
            Sleepy::loseSomeTime(oddOff);
